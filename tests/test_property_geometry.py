...
def test_checkpoint_camera_to_world_is_normalized_before_unprojection():
    ...
    decoded = np.column_stack([rotation, center])
    normalized = world_to_camera(decoded, "camera-to-world")
    point = unproject(np.array([[3.0]]), np.eye(3), normalized)[0, 0]
    np.testing.assert_allclose(point, center + rotation @ [0, 0, 3])
    np.testing.assert_allclose(
        world_to_camera(normalized, "world-to-camera"), normalized
    )
...
def test_opencv_unprojection_uses_camera_z_and_world_to_camera():
    ...
    k = np.array([[10.0, 0, 1], [0, 20.0, 1], [0, 0, 1.0]])
    points = unproject(np.full((3, 3), 2.0), k, extrinsic)
    np.testing.assert_allclose(
        points[1, 1], center + rotation.T @ np.array([0, 0, 2.0])
    )
...
def test_registration_recovers_scale_and_rotation_with_outliers():
    ...
    truth = np.eye(4)
    truth[:3, :3] = 1.7 * Rotation.from_euler("xyz", [0.2, -0.4, 0.7]).as_matrix()
    truth[:3, 3] = [3, -4, 2]
    target = transform_points(source, truth) + rng.normal(0, 0.001, source.shape)
    target[:80] += rng.normal(0, 2, (80, 3))
    fit, _ = robust_similarity(source, target)
    np.testing.assert_allclose(fit, truth, atol=0.003)


def test_multiview_filter_rejects_depth_inconsistent_patch():
    k = np.array([[30.0, 0, 16], [0, 30.0, 16], [0, 0, 1.0]], np.float32)
    depth = np.full((5, 32, 32), 2, np.float32)
    depth[2, 8:16, 8:16] = 3
    rng = np.random.default_rng(1)
    data = {
        "depth": depth,
        "intrinsics": np.repeat(k[None], 5, 0),
