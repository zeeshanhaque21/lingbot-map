...
def test_colmap_refinement_calibrates_depth_and_preserves_global_cameras(tmp_path):
    ...
    for y in range(7):
        for x in range(7):
            identifier = 1 + y * 7 + x
            point_rows.append(
                f"{identifier} {(x - 3) * 0.2} {(y - 3) * 0.2} 2 100 100 100 0.1 1 0 2 0 3 0"
            )
            observations.append(f"{x} {y} {identifier}")
    (model / "points3D.txt").write_text("\n".join(point_rows) + "\n")
    (model / "images.txt").write_text(
        "".join(
            f"{i + 1} 1 0 0 0 0 0 0 1 {i:06d}.jpg\n" + " ".join(observations) + "\n"
            for i in range(8)
        )
    )
    ...
    assert not json.loads((destination / "depth-calibration.json").read_text())[
        "metric_scale_known"
    ]
