...
def read_model(path):
    ...
    for line in (path/"cameras.txt").read_text().splitlines():
        ...
        fields=line.split()
        if fields[1]!="PINHOLE":
            raise ValueError("Camera refinement requires PINHOLE images; undistort other camera models first")
        fx,fy,cx,cy=map(float,fields[4:])
        cameras[int(fields[0])] = {"width":int(fields[2]),"height":int(fields[3]),
    ...
    for line in lines:
        ...
        images[fields[9]]={"id":int(fields[0]),"camera":cameras[int(fields[8])],
                            "extrinsics":extrinsic,"observations":observations}
    for line in (path/"points3D.txt").read_text().splitlines():
        if not line.strip() or line.startswith("#"):continue
        fields=line.split()
        points[int(fields[0])] = {"xyz":np.asarray(fields[1:4],float),"error":float(fields[7]),
