...
def refine(source, destination, colmap_model):
    ...
    if destination.exists():
        raise ValueError(
            "Refinement output exists; choose a new directory to preserve the previous model"
        )
    ...
    configuration.update(
        {
            "poses_global": True,
            "pose_source": "COLMAP feature tracks and bundle adjustment",
    ...
    for window in windows:
        ...
        for index, frame in enumerate(data["frame_ids"]):
            ...
            valid = [
                ...
                if int(o[2]) in points
                and points[int(o[2])]["error"] < 2
                and points[int(o[2])]["track_length"] >= 3
            ]
            if len(valid) < 30:
                continue
            ...
            fit = {
                "frame": int(frame),
                "window": int(window.stem),
                "sparse_points": int(keep.sum()),
                "depth_scale": scale,
                "median_sparse_relative_depth_error": float(np.median(error)),
                "p95_sparse_relative_depth_error": float(np.quantile(error, 0.95)),
            }
            fit["accepted"] = fit["median_sparse_relative_depth_error"] < 0.15
            fits.append(fit)
            if not fit["accepted"]:
                continue
    ...
    write_json(
        destination / "depth-calibration.json",
        {
            "fits": fits,
