...
def infer(
    ...
    if device == "mps" and not torch.backends.mps.is_available():
        raise ValueError(
            "MPS unavailable; use --device cpu or an Apple Silicon Python installation"
        )
    from .poses import LONG_CHECKPOINT_SHA256, world_to_camera

    checkpoint_hash = digest(checkpoint)
    if pose_convention == "auto":
        if checkpoint_hash != LONG_CHECKPOINT_SHA256:
            raise ValueError(
                "Unknown checkpoint pose convention; specify --pose-convention after validation"
            )
    ...
    configuration = {
        ...
        "schema": 2,
        "checkpoint_pose_convention": pose_convention,
    ...
    for start, end in window_ranges(len(frames), window, overlap):
        ...
        if model is None:
            ...
            if device == "mps":
                ...
                def forward_with_memory_release(
                    *args, _forward=original_forward, _trace=trace, **kwargs
                ):
        ...
        with torch.inference_mode(), amp:
            prediction = model.inference_streaming(
                images,
                num_scale_frames=min(8, end - start),
        ...
        if not np.isfinite(depth).all() or not np.isfinite(extrinsics.numpy()).all():
            raise ValueError(
                f"Non-finite model output at frame {start}; try --precision float32"
            )
        write_npz(
            destination,
            ...
            extrinsics=world_to_camera(extrinsics[0].numpy(), pose_convention),
            intrinsics=intrinsics[0].numpy(),
            rgb=(images.permute(0, 2, 3, 1).numpy() * 255)
            .round()
            .clip(0, 255)
            .astype(np.uint8),
        )
