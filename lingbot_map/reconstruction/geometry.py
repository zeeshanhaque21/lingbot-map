...
def robust_similarity(source, target):
    ...
    return transform, np.linalg.norm(transform_points(source, transform)-target, axis=1)


def align_overlap(previous, current):
    common, old_ids, new_ids = np.intersect1d(previous["frame_ids"], current["frame_ids"],
                                             return_indices=True)
    if len(common) < 3:
        raise ValueError("Windows have fewer than three shared frames")
    source, target = [], []
    for old, new in zip(old_ids, new_ids):
        a = unproject(previous["depth"][old], previous["intrinsics"][old], previous["extrinsics"][old])
        b = unproject(current["depth"][new], current["intrinsics"][new], current["extrinsics"][new])
        valid = (previous["confidence"][old] > np.quantile(previous["confidence"][old], .5))
        valid &= current["confidence"][new] > np.quantile(current["confidence"][new], .5)
    ...
    stats = {"shared_frames": len(common), "correspondences": len(source),
             "scale": float(np.cbrt(np.linalg.det(transform[:3,:3]))),
             "median_relative_error": float(np.median(errors)/reference),
             "p95_relative_error": float(np.quantile(errors,.95)/reference)}
    if not .5 < stats["scale"] < 2 or stats["median_relative_error"] > .08:
        raise ValueError(f"Unreliable overlap registration: {stats}")
    return transform, stats


def supported_depth(index, data, relative_tolerance=.03):
    ...
    valid = np.isfinite(depth) & (depth > 0) & (confidence > np.quantile(confidence,.25))
    high = cv2.dilate(depth, np.ones((3,3),np.uint8))
    low = cv2.erode(depth, np.ones((3,3),np.uint8))
    valid &= (high-low) < .08*np.maximum(depth,1e-6)
    ...
    for neighbor in (index-3,index-1,index+1,index+3):
        ...
        observed = cv2.remap(data["depth"][neighbor],u.astype(np.float32),v.astype(np.float32),cv2.INTER_NEAREST)
        observed_conf = cv2.remap(data["confidence"][neighbor],u.astype(np.float32),v.astype(np.float32),cv2.INTER_NEAREST)
        projected_valid = valid & (camera[...,2]>0) & (observed>0) & (observed_conf>np.quantile(data["confidence"][neighbor],.25))
        ...
        visible = projected_valid & (camera[...,2] <= observed*(1+relative_tolerance))
    ...
    stats = {"frame": int(data["frame_ids"][index]), "pixels": int(depth.size),
             "candidate_pixels": int(valid.sum()), "accepted_pixels": int(accepted.sum()),
             "projected_comparisons": tested,
             "median_visible_relative_residual": float(np.median(errors)) if errors else None}
